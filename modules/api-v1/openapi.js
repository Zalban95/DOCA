  ...
  const P = {
    ...
    'agent.turn':       { audience: 'device', payload: obj({
      ...
      fallbacks: arr(obj({
        ...
        from: str({ description: 'Provider that went quiet — the one that stalled, or the configured model passed over because it stalled a moment ago.' }), fromModel: str(),
        to: str({ description: 'Provider that answered instead.' }), toModel: str(),
        seconds: int({ description: 'How long the silent one was given before the hop — or, where the hop passed over a model that stalled in an earlier turn, how long ago it stalled.' }),
      }), { description: 'On `done`: present only when the fallback chain moved the turn to another model — including a turn that never reached the configured model, because it stalled in the last few minutes.' }),
  ...
  return {
    ...
    '/devices/{id}': {
      ...
      delete: { tags: ['Devices'], summary: 'Revoke a device (token dies, streams close, outbox and profile deleted)', operationId: 'revokeDevice', ...scopeDoc('devices:admin'),
        parameters: [{ name: 'purge', in: 'query', schema: str(), description: 'Set to `1` to also forget the device: the registry row is deleted, not just marked revoked. Revoking keeps the row as the audit trail; this removes it and everything kept under the id.' }],
        responses: { 200: json(obj({ ok: bool(), deviceId: str(), purged: bool(), revokedAt: iso() })), ...std(400, 401, 403, 404) } },
    },
    ...
    '/harness/missions': { get: { tags: ['Harness'], summary: 'Missions the specialist agents are running', operationId: 'harnessListMissions', ...scopeDoc('harness:chat'),
      ...
      parameters: [{ name: 'state', in: 'query', schema: str({ description: 'running | paused | done | failed | cancelled' }) }, { name: 'limit', in: 'query', schema: int({ description: 'Default 20, maximum 50.' }) }, { name: 'all', in: 'query', schema: str({ description: '1 to include archived missions, which are otherwise left out so every client agrees about what is still open.' }) }],
      responses: { 200: json(obj({ enabled: bool(), missions: arr(obj({ id: str(), agentId: str(), label: str(), task: str(), state: str(), steps: int(), tokens: int(), startedAt: iso(), endedAt: nullable(iso()), archivedAt: nullable(iso({ description: 'Put away by the user. Kept, with its log, and left out of this list unless all=1.' })), result: nullable(str()), error: nullable(str()) })) })), ...std(401, 403) } } },
